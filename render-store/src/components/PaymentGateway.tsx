...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .pg-merchant-name {
    font-size: 15px; font-weight: 600; color: #fff; display: block;
    line-height: 1.25;
  }
  .pg-merchant-sub {
    font-size: 11px; color: rgba(255,255,255,0.75); display: block; margin-top: 2px;
    line-height: 1.35;
...
/** Passed when the user submits “Confirm your payment” (manual UPI details). */
export interface PaymentManualSubmitDetails {
  name: string;
  email: string;
  utr: string;
  /** Same reference shown in UPI intent (e.g. UPI/2603/…). */
  referenceId: string;
}
...
export interface PaymentGatewayProps {
  ...
  /** Called after validation; should confirm payment with the API, then place the order. */
  onPaymentSuccess: (details: PaymentManualSubmitDetails) => Promise<void>;
}
...
function getAppDeepLink(appId: string, queryString: string) {
  const withQs = (scheme: string) => `${scheme}${queryString}`;
  switch (appId) {
    case "gpay":
...
export default function PaymentGateway({
  ...
  orderId,
  itemSummaryLine,
  merchantVpa = (import.meta.env.VITE_MERCHANT_UPI as string | undefined) || "merchant@upi",
  merchantInitials,
  onPaymentSuccess,
}: PaymentGatewayProps) {
  ...
  const [upiValidity, setUpiValidity] = useState<"idle" | "valid" | "invalid">("idle");
  ...
  const [paidFormErrors, setPaidFormErrors] = useState<Record<string, string>>({});
  ...
  const amountInrStr = useMemo(() => (amountPaise / 100).toFixed(2), [amountPaise]);
  const amountMainDisplay = useMemo(
    () =>
  ...
  const avatarLetters = merchantInitials ?? merchantName.slice(0, 2).toUpperCase();

  const openUpiPayment = useCallback(
    (appId: string | null) => {
      if (!appId) return;
    ...
    [refNo, merchantVpa, merchantName, amountInrStr, orderId]
  );

  const qrUpiPayload = useMemo(
    () =>
      `upi://pay?${buildUpiQueryString({
        pa: merchantVpa,
        pn: merchantName,
        am: amountInrStr,
        cu: "INR",
    ...
    [merchantVpa, merchantName, amountInrStr, orderId, refNo]
  );
  ...
  const validateUpi = useCallback((val: string) => {
    if (!val) {
      setUpiValidity("idle");
      return;
    }
  ...
  const handlePaidFormSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      await onPaymentSuccess({
        ...
        utr: payerUtr.replace(/\s/g, ''),
        referenceId: refNo,
      });
      setSuccess(true);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : "Order could not be placed. Try again.";
      setPaidFormApiError(msg);
      setShowPaidForm(true);
    } finally {
      setProcessing(false);
    }
          ...
          {success ? (
            <div className="success-screen">
              ...
              <div className="success-sub">
                {formatINR(amountPaise)} paid to {merchantName}
              </div>
            ...
            <>
              ...
              <div className="pg-header">
                <div className="pg-merchant-row">
                  <div className="pg-avatar">{avatarLetters}</div>
                  <div>
                    <span className="pg-merchant-name">{merchantName}</span>
                    <span className="pg-merchant-sub">Order #{orderId} · {itemSummaryLine}</span>
                  </div>
                </div>
              ...
              <div className="pg-body">
                ...
                {tab === "qr" && (
                  <div className="qr-wrap">
                    ...
                    <div className="qr-upi-id">Scan to pay · <strong>{merchantVpa}</strong></div>
                ...
                {tab === "apps" && (
                  <>
                    ...
                    <div className="app-hint">
                      {selectedApp
                        ? `Opens ${UPI_APPS.find((a) => a.id === selectedApp)?.name} with ${formatINR(amountPaise)} for ${merchantName}. Tap Pay to open again.`
                ...
                {tab === "id" && (
                  <>
                    ...
                    >
                      {processing ? <><span className="spinner"/>Processing...</> : `Pay ${formatINR(amountPaise)}`}
                    </button>
                  </>
                )}
              </div>
