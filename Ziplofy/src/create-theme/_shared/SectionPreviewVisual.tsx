...
export function SectionPreviewVisual({ variant }: { variant: SectionPreviewVariant }) {
    ...
    return (
      <HeroSceneFrame>
        ...
        <div className="absolute inset-x-0 bottom-[14%] flex justify-center">
          <span className="rounded-md bg-white px-5 py-2 text-xs font-medium text-gray-900 shadow-md">
            Shop now
          </span>
        </div>
        <style>{`
  ...
  if (variant === 'divider') {
    return (
      <div
        className="mx-auto flex h-[min(280px,38vh)] w-full max-w-[560px] items-center justify-center bg-[#e8e8e8]"
        role="img"
        aria-label="Divider section preview"
      >
        <div className="h-2.5 w-[90%] max-w-[500px] bg-white" role="presentation" />
      </div>
    );
  }
