...
import { Alert, Box, Button, Card, CardContent, Dialog, DialogActions, DialogContent, DialogTitle, Divider, Drawer, FormControl, IconButton, InputAdornment, InputLabel, MenuItem, Select, Stack, TextField, Typography } from '@mui/material';
...
import { useAmountOffOrder } from '../contexts/storefront/amount-off-order.context';
import { useFreeShipping } from '../contexts/storefront/storefront-free-shipping.context';
...
const CartDrawer: React.FC<CartDrawerProps> = ({ open, onClose }) => {
  ...
  const { eligibleDiscounts, loading: discountsLoading, error: discountsError, fetchEligibleDiscounts, amountOffOrderDiscountCodeCheck } = useAmountOffOrder();
  const { 
    eligibleDiscounts: freeShippingDiscounts, 
    discountCodeResult: freeShippingCodeResult, 
    loading: freeShippingLoading, 
    error: freeShippingError, 
    discountCodeLoading: freeShippingCodeLoading, 
    discountCodeError: freeShippingCodeError, 
    checkEligibleFreeShippingDiscounts,
    validateFreeShippingDiscountCode, 
    clearDiscountCodeResult: clearFreeShippingCodeResult 
  } = useFreeShipping();
  const { addresses, fetchCustomerAddressesByCustomerId, loading: addressesLoading } = useCustomerAddresses();
  ...
  const addressesFetchedRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    addressesFetchedRef.current = null;
  }, [user?._id]);
  ...
  const validateDiscountCode = useCallback(async (code: string) => {
    if (!code.trim() || !user?._id || !storeFrontMeta?.storeId || items.length === 0) {
    ...
    try {
      ...
      const result = await amountOffOrderDiscountCodeCheck(
  ...
  const validateFreeShippingCode = useCallback(async (code: string) => {
    ...
    try {
      ...
      const defaultAddress = user?.defaultAddress ? addresses.find(addr => addr._id === user.defaultAddress) : null;
      await validateFreeShippingDiscountCode({
        storeId: storeFrontMeta.storeId,
        customerId: user._id,
        cartItems,
      ...
      console.error('Error validating free shipping code:', error);
    }
  ...
  const handleDiscountCodeChange = useCallback((value: string) => {
    ...
    debounceTimerRef.current = window.setTimeout(() => { validateDiscountCode(value); }, 500);
  }, [validateDiscountCode]);

  const handleFreeShippingCodeChange = useCallback((value: string) => {
    setFreeShippingCode(value);
    if (freeShippingDebounceTimerRef.current) clearTimeout(freeShippingDebounceTimerRef.current);
    freeShippingDebounceTimerRef.current = window.setTimeout(() => { validateFreeShippingCode(value); }, 500);
  }, [validateFreeShippingCode]);
  ...
  const discountAmount = React.useMemo(() => {
    if (discountCodeResult) return discountCodeResult.discountAmount || 0;
    if (selectedDiscount) return selectedDiscount.discountAmount || 0;
    return 0;
  }, [discountCodeResult, selectedDiscount]);
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      const orderPayload = { storeId: storeFrontMeta.storeId, shippingAddressId: selectedShippingAddressId, billingAddressId: selectedBillingAddressId || undefined, items: orderItems, paymentMethod, subtotal, tax, shippingCost, total: finalTotal + shippingCost + tax };
                      ...
                      <CardContent sx={{ p: 2 }}>
                        <Stack direction="row" spacing={2} alignItems="center">
                          ...
                          <Stack spacing={1} alignItems="flex-end" sx={{ minWidth: 140 }}>
                            ...
                            <IconButton color="error" onClick={() => deleteCartEntry(it._id)} size="small">
                              <DeleteIcon />
                            </IconButton>
                          </Stack>
                        </Stack>
                ...
                <Stack spacing={0.5}>
                  ...
                  {discountAmount > 0 && (
                    <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                      ...
                      <Typography variant="body2" color="success.main">-${(discountAmount/100).toFixed(2)}</Typography>
                    </Box>
                  )}
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Typography variant="body2" color="text.secondary">Shipping</Typography>
                    <Typography variant="body2">${(shippingCost/100).toFixed(2)}</Typography>
                  </Box>
                  {tax > 0 && (
                    <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <Typography variant="body1" fontWeight={700}>${((finalTotal + shippingCost + tax)/100).toFixed(2)}</Typography>
                  </Box>
                </Stack>
              </Box>
            </Stack>
