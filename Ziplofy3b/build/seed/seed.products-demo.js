...
function variantCombos(optionDefs) {
    return optionDefs.reduce((acc, opt) => {
        if (acc.length === 0)
...
function buildVariantDocs(productId, body) {
...
const DEMO_PRODUCTS = [
    {
        ...
        description: 'Soft cotton tee with a relaxed fit. Demo product with color and size options for cartesian variants.',
...
async function ensureRefs(storeId) {
    const store = await store_model_1.Store.findById(storeId);
    if (!store) {
        throw new Error(`Store not found: ${storeId.toHexString()}`);
    }
    const locations = await location_model_1.LocationModel.find({ storeId }).select('_id');
    if (locations.length === 0) {
        throw new Error(`No locations for store ${storeId.toHexString()}. Create a location before seeding products.`);
    }
    const category = await category_model_1.Category.findOne().sort({ createdAt: 1 });
    if (!category) {
        throw new Error('No categories in DB. Run npm run seed:categories first.');
    }
    const storeIdStr = storeId.toHexString();
    let productType = await product_type_model_1.ProductType.findOne({ storeId: storeIdStr });
    if (!productType) {
        productType = await product_type_model_1.ProductType.create({ storeId: storeIdStr, name: 'General' });
        console.log('Created ProductType "General" for store.');
    }
    let vendor = await vendor_model_1.Vendor.findOne({ storeId });
    if (!vendor) {
        vendor = await vendor_model_1.Vendor.create({ storeId, name: 'Seed Demo Vendor' });
        console.log('Created Vendor "Seed Demo Vendor" for store.');
    }
    return {
        categoryId: category._id,
        productTypeId: new mongoose_1.default.Types.ObjectId(String(productType._id)),
        vendorId: vendor._id,
        locationIds: locations.map((l) => l._id),
    };
}
async function removePreviousDemo(storeId, urlHandles) {
    const existing = await product_model_1.Product.find({ storeId, urlHandle: { $in: urlHandles } }).select('_id');
    const productIds = existing.map((p) => p._id);
    if (productIds.length === 0)
        return;
    const variants = await product_variants_model_1.ProductVariant.find({ productId: { $in: productIds } }).select('_id');
    const variantIds = variants.map((v) => v._id);
    if (variantIds.length > 0) {
        await inventory_level_model_1.InventoryLevelModel.deleteMany({ variantId: { $in: variantIds } });
    }
    await product_variants_model_1.ProductVariant.deleteMany({ productId: { $in: productIds } });
    await product_model_1.Product.deleteMany({ _id: { $in: productIds } });
    console.log(`Removed ${productIds.length} prior demo product(s) and related variants/inventory.`);
}
async function seedOneProduct(def, storeId, refs) {
    ...
    const inventoryLevelDocs = createdVariants.flatMap((variant) => refs.locationIds.map((locId) => ({
    ...
    console.log(`  ✓ ${def.title} — ${createdVariants.length} variant(s), ${inventoryLevelDocs.length} inventory row(s)`);
}
async function main() {
    const raw = process.env.SEED_STORE_ID || DEFAULT_STORE_ID;
    if (!mongoose_1.default.Types.ObjectId.isValid(raw)) {
        throw new Error(`Invalid SEED_STORE_ID / default store id: ${raw}`);
    }
    const storeId = new mongoose_1.default.Types.ObjectId(raw);
