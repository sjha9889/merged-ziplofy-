...
/**
 * Testimonials strip with circular avatars and side arrows.
 */
          ...
          <div className="flex min-w-max gap-8 pr-6">
            {testimonials.map((t, idx) => (
              <article key={`${t.name}-${idx}`} className="w-[220px]">
                ...
                />
                <h3 className="mt-4 text-[34px] font-medium leading-none text-black">{t.name}</h3>
                <p className="mt-2 text-[14px] font-normal text-[#7a7a7a]">{t.role}</p>
